...
def main():
    ...
    # ----------------------------------------
    model_name = 'drunet'
    quality_factors = [75]              # 重新训练时，quality_factors无效
    # testset_name = 'BOWS'            # test set,  'classic5'  | 'LIVE1'
    testset_name = '/data2/wushihang/liweixiang/Steganalysis/Data/lxl_drunet_train/Alaska2-4w/Alaska2-40000-QF75'
    need_degradation = False              # default: True

    task_current = 'Alaska2-QF75-3w'       # 'db' for JPEG image deblocking

    model_pool = 'model_zoo'  # fixed
    testsets = '/data2/wushihang/liweixiang/Steganalysis/Data'     # fixed
    results = '/data2/wushihang/liweixiang/Steganalysis/Data/lxl_drunet_train/drunet_Alaska2-4w'       # fixed
    result_name = results + '/' + model_name + '_' + task_current
    ...
    if model_name == 'dncnn3':
        ...
        model_name = 'drunet'
        model_path = os.path.join('model_zoo', '30000_G.pth')          # 模型放在model_zoo文件夹下
        from models.network_unet import UNetRes as net
        model = net(in_nc=1, out_nc=1, nc=[64, 128, 256, 512], nb=4, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose')
