...
def main():
    ...
    parser.add_argument('--jpeg', type=str, default=40, help='scale factor: 10, 20, 30, 40')
...
def define_model(args):
    ...
    elif args.task == 'jpeg_bows':
        model = net(upscale=1, in_chans=1, img_size=126, window_size=7,
                    img_range=255., depths=[6, 6, 6, 6, 6, 6], embed_dim=180, num_heads=[6, 6, 6, 6, 6, 6],
                    mlp_ratio=2, upsampler='', resi_connection='1conv')
        param_key_g = 'params'
    elif args.task == 'jpeg_ala':
        model = net(upscale=1, in_chans=1, img_size=126, window_size=7,
                    img_range=255., depths=[6, 6, 6, 6, 6, 6], embed_dim=180, num_heads=[6, 6, 6, 6, 6, 6],
                    mlp_ratio=2, upsampler='', resi_connection='1conv')
        param_key_g = 'params'
...
def setup(args):
    ...
    elif args.task in ['jpeg_bows']:
        save_dir = f'results/swinir_{args.task}_jpeg{args.jpeg}'
        folder = args.folder_lq
        border = 0
        window_size = 7
        
    elif args.task in ['jpeg_ala']:
        save_dir = f'results/swinir_{args.task}_jpeg{args.jpeg}'
        folder = args.folder_lq
        border = 0
        window_size = 7
