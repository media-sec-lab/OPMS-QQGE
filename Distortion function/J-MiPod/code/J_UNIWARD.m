 function [ S_STRUCT , pChange , ChangeRate ] = J_UNIWARD(C_STRUCT , C_SPATIAL , payload )
...
% algorithm.
% -------------------------------------------------------------------------
% Input:  C_STRUCT ..... Resulting stego jpeg STRUCT with embedded payload
%         C_SPATIAL .... Image in spatial domain
%         payload ...... Payload in bits per non zero DCT coefficient
% Output: S_STRUCT ..... Resulting JPEG structure with embedded payload
...
%messageLenght = round(payload * nzAC);
%    fprintf("Message lenght = %d ; nbAC = %d \n", messageLenght, nzAC);

%% Embedding simulation
% S_COEFFS = C_COEFFS;
[ S_COEFFS, pChangeP1, pChangeM1] = EmbeddingSimulator(C_COEFFS, rhoP1, rhoM1, round(payload * nzAC));

S_STRUCT = C_STRUCT;
S_STRUCT.coef_arrays{1} = S_COEFFS;

pChange = pChangeP1 + pChangeM1;
pChange = reshape(pChange, size(S_COEFFS));

ChangeRate = sum(pChange(:))/numel(C_COEFFS); % Computing the change rate
