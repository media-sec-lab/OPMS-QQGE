function [ S_STRUCT , pChange , ChangeRate ] = UERD_Emb(C_STRUCT, payload)
% -------------------------------------------------------------------------
...
DCT_coefs_padded(9:end-8, 9:end-8) = DCT_coefs(:,:);
DCT_coefs_padded(1:8, :) = DCT_coefs_padded(9:16, :);
DCT_coefs_padded(end-7:end, :) = DCT_coefs_padded(end-15:end-8, :);
DCT_coefs_padded( : , 1:8, :) = DCT_coefs_padded( : , 9:16);
DCT_coefs_padded( : , end-7:end, :) = DCT_coefs_padded( : , end-15:end-8);
...
[S_COEFFS pChangeP1 pChangeM1] = EmbeddingSimulator(DCT_coefs, rhoP1, rhoM1, round(payload * nzAC) );
